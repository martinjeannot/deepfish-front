<template>
  <v-app v-if="appCreated">
    ...
    <v-navigation-drawer app fixed temporary v-model="showSideNav">
      <v-list>
        <v-list-tile v-for="menuItem in menuItems" :key="menuItem.title" router
    ...
    <v-toolbar app>
      ...
      <v-toolbar-items class="hidden-xs-only">
        <v-btn flat v-for="menuItem in menuItems" :key="menuItem.key" router
          :to="menuItem.route" exact>
          <v-badge color="red">
    ...
    <v-footer app dark height="auto">
      <v-card flat tile width="inherit" class="text-xs-center" color="#3A3A3A">
        <v-card-text class="py-0">
          <v-btn v-for="link in footerLinks" :key="link.href" icon class="mx-3"
          ...
          menuItems = [
            {
              ...
              icon: 'assignment',
              title: 'Mes besoins',
              route: '/employer/requirements',
            },
            {
              key: 'talents',
              icon: 'assignment_ind',
              title: 'Mes talents',
              route: '/employer/talents',
            },
            {
              key: 'profile',
              icon: 'account_circle',
