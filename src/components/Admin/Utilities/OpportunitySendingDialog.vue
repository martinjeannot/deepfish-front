      ...
      <v-form v-model="valid" @submit.prevent="sendOpportunity">
        <v-layout>
          <v-flex xs12>
            ...
            <v-select label="Requirement" :items="requirements" v-model="selectedRequirement"
              ...
              <template slot="item" slot-scope="data">
                <v-list-tile>
                  <v-list-tile-content>
                    <v-list-tile-title>
                      <span style="font-weight: bold">{{ data.item.company.name }}</span> : {{ data.item.name }}
                    </v-list-tile-title>
                  </v-list-tile-content>
                </v-list-tile>
              </template>
...
export default {
  ...
  methods: {
    ...mapActions(['prepareForApiConsumption', 'clearLoading', 'showSnackbar']),
    sendOpportunity() {
